(() => {
  ...
  const DEFAULT_ROUTES = [
    ...
    'codex',
    'wiki/hero-database',
    'wiki/progression-planning',
    'wiki/events-overview',
    'wiki/resource-source-index',
    'wiki/research-center-overview',
    'wiki/alliance-systems-overview',
    'wiki/season-4-sealed-island'
  ];
  ...
  function categoryOf(entry) {
    ...
    if (route === 'charter' || route === 'codex') return 'Governance & Operations';
    if (route === 'wiki' || route.startsWith('wiki/')) return 'Game Wiki';
    return entry.label || 'Other';
  }

  function prepared(entry) {
    const aliases = Array.isArray(entry.aliases) ? entry.aliases : [];
    const title = normalize(entry.heading);
  ...
  function scoreEntry(item, query) {
    ...
    qTokens.forEach(token => {
      ...
      if (item.titleWords.includes(token)) {
        score += 28;
        tokenMatched = true;
      } else if (item.title.includes(token)) {
        score += 17;
        tokenMatched = true;
      }

      if (item.aliasText.includes(token)) {
        score += 22;
        tokenMatched = true;
      }
      if (item.label.includes(token)) score += 8;
      if (item.body.includes(token)) {
        score += 4;
        tokenMatched = true;
      }
      ...
      if (!tokenMatched && token.length >= 4) {
        ...
        const fuzzy = item.titleWords.some(candidate => {
          if (Math.abs(candidate.length - token.length) > allowance) return false;
          return levenshtein(token, candidate) <= allowance;
        });
        if (fuzzy) {
          score += token.length >= 7 ? 14 : 10;
          tokenMatched = true;
        }
      }
  ...
  function ensureFilterHost() {
    ...
    filterHost.setAttribute('aria-label', 'Search category filters');
  ...
  function renderFilters() {
    ...
    host.innerHTML = [
      `<button type="button" data-search-category="ALL" class="${activeCategory === 'ALL' ? 'active' : ''}">All</button>`,
    ...
    host.querySelectorAll('[data-search-category]').forEach(button => {
  ...
  function defaultMatches(entries) {
    ...
    entries.forEach(entry => {
      if (selected.length >= 10 || selected.includes(entry)) return;
      selected.push(entry);
    });
    return selected;
  }
  ...
  function enhancedRenderSearchResults(query) {
    ...
    let matches;
    if (!q) {
      matches = defaultMatches(filteredByCategory);
    } else {
      matches = filteredByCategory
        .map(prepared)
        .map(item => ({ entry: item.entry, category: item.category, score: scoreEntry(item, q) }))
    ...
    results.innerHTML = matches.length
      ? matches.map(entry => {
          ...
          return `<a class="search-result search-result-ranked" href="#/${escapeHtml(entry.route)}" data-search-link>
            ...
            <p>${escapeHtml(excerpt(entry, q))}</p>
          </a>`;
        }).join('')
      : `<div class="empty-state">No matching results${activeCategory !== 'ALL' ? ` in ${escapeHtml(activeCategory)}` : ''}. Try a shorter term, an abbreviation, or another category.</div>`;

    results.querySelectorAll('[data-search-link]').forEach(anchor => {
      anchor.addEventListener('click', () => dialog.close());
    });
  }

  try {
    renderSearchResults = enhancedRenderSearchResults;
  } catch (_) {
